...
async def download_fiduciary_short_pdf(
    ...
    """Compile and stream the actual raw official Form 5500-SF document directly for a plan audit."""
    try:
        tenant_id = await resolve_tenant_id(db, current_user)
        ...
        if not record:
            ...
            if prospect:
                ...
                record_clean = {
                    ...
                    "fee_flag": False,
                    "participation_flag": False,
                    "administrator_name": "Plan Fiduciary Board"
                }
            else:
                raise HTTPException(status_code=404, detail=f"No DOL filing or Excel prospect details found for EIN {clean_ein}.")
        else:
        ...
        from utils.pdf_generator import compile_raw_form_5500_sf_pdf
        from fastapi.responses import StreamingResponse
        
        pdf_stream = compile_raw_form_5500_sf_pdf(record_clean)
